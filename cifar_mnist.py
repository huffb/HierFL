...
cudnn.banchmark = False#2024.4.22 rewrite
...
def split_data(dataset, args, kwargs, is_shuffle = True):
    ...
    """
    if args.iid == 1:
        data_loaders = iid_esize_split(dataset, args, kwargs, is_shuffle) #iid
    elif args.iid == 0:
        data_loaders = niid_esize_split(dataset, args, kwargs, is_shuffle)#noniid等量
    elif args.iid == -1:
        data_loaders = iid_nesize_split(dataset, args, kwargs, is_shuffle)#iid不等量
    elif args.iid == -2:
        data_loaders = niid_esize_split_oneclass(dataset, args, kwargs, is_shuffle)#极端iid
    else :
...
def get_dataset(dataset_root, dataset, args):
    ...
    if dataset == 'mnist':
        ...
        train_loaders, test_loaders, v_train_loader, v_test_loader = get_fmnist(dataset_root, args)
    else:
...
def get_fmnist(dataset_root, args):
    is_cuda = args.cuda
    kwargs = {'num_workers': 1, 'pin_memory': True} if is_cuda else {}
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,)),  # Fashion-MNIST 的标准化值
    ])
    train = datasets.FashionMNIST(os.path.join(dataset_root, 'fmnist'), train = True,
                            download = True, transform = transform)
    test =  datasets.FashionMNIST(os.path.join(dataset_root, 'fmnist'), train = False,
                            download = True, transform = transform)
    #note: is_shuffle here also is a flag for differentiating train and test
    train_loaders = split_data(train, args, kwargs, is_shuffle = True)
    test_loaders = split_data(test,  args, kwargs, is_shuffle = False)
